...
begin
    ...
    -- Font ROM lookup for the selected character and font pixel.
    rom_inst : char_rom
        port map(
    ...
    -- Select the active character and font pixel for the current VGA coordinate.
    ...
    begin
        ...
        case text_scale is
            when "00" =>
                row_end := py + 8;
                col_end := px + text_len * 8;

            when "01" =>
                row_end := py + 16;
                col_end := px + text_len * 16;

            when "10" =>
                row_end := py + 32;
                col_end := px + text_len * 32;

            when others =>
                row_end := py + 64;
                col_end := px + text_len * 64;
        end case;

        if (
            to_integer(row_ref) >= py and to_integer(row_ref) < row_end and
            to_integer(col_ref) >= px and to_integer(col_ref) < col_end
        ) then
            in_region <= '1';
