...
ARCHITECTURE structural OF de0_cv_default IS
    ...
    SIGNAL dummy_rgb : std_logic_vector(2 downto 0);
...
BEGIN
    ...
    -- VGA SYNC (Outputs directly to the physical sync pins)
    vga_sync_inst : ENTITY work.vga_sync PORT MAP(
            clock_25mhz => clk_25mhz, 
            red => '1', green => '1', blue => '1', -- Dummy inputs as color routing is done below
            red_out => open, green_out => open, blue_out => open, 
    ...
    lfsr_inst : ENTITY work.lfsr PORT MAP (
            clk => clk_25mhz, reset => reset_sys, q_out => lfsr_rand);
    ...
    bird_inst : ENTITY work.bird_logic PORT MAP(
            ...
            reset_pos => reset_game, game_state => game_state,
            pixel_row => pixel_row, pixel_column => pixel_col, bird_on_out => bird_on, bird_y_out => bird_y);
    ...
    level_inst : ENTITY work.conveyor_belt PORT MAP(
            ...
            current_lvl => current_level, lfsr_rand => lfsr_rand,
            pixel_row => pixel_row, pixel_col => pixel_col, bird_y => bird_y,
            pipe_on => pipe_on, ground_on => ground_on, gift_on => gift_on, 
            collision => collision, score_tick => score_tick, gift_tick => gift_tick);

    -- TEXT OVERLAYS
    t_menu : ENTITY work.text_engine 
				GENERIC MAP ("MAIN MENU")
				PORT MAP(clk_25mhz => clk_25mhz, pixel_row => pixel_row, pixel_col => pixel_col,
					position_x => "0011001000", position_y => "0011001000", text_scale => "10",
					text_on => text_menu, text_rgb => dummy_rgb);
	
    t_train : ENTITY work.text_engine 
				GENERIC MAP ("TRAINING") 
				PORT MAP(clk_25mhz => clk_25mhz, pixel_row => pixel_row, pixel_col => pixel_col,
					position_x => "0000000000", position_y => "0000000000", text_scale => "01",
					text_on => text_train, text_rgb => dummy_rgb);
                    
    t_game : ENTITY work.text_engine 
				GENERIC MAP ("GAME")
				PORT MAP(clk_25mhz => clk_25mhz, pixel_row => pixel_row, pixel_col => pixel_col,
					position_x => "0000000000", position_y => "0000000000", text_scale => "01",
					text_on => text_game, text_rgb => dummy_rgb);
                    
    t_over : ENTITY work.text_engine 
				GENERIC MAP ("GAME OVER")
				PORT MAP(clk_25mhz => clk_25mhz, pixel_row => pixel_row, pixel_col => pixel_col,
					position_x => "0011001000", position_y => "0011001000", text_scale => "10",
					text_on => text_over, text_rgb => dummy_rgb);

    -- Toggle wording based on the DIP switch
    text_play <= text_train WHEN sw(9) = '1' ELSE text_game;

    -- NEW 4-BIT PIXEL ROUTER 
    router_inst : ENTITY work.display_pixel_router PORT MAP(
            game_state => game_state,
			sw_training => sw(9),
            text_menu_on => text_menu,
			text_play_on => text_play,
			text_over_on => text_over,
            bird_on => bird_on,
			pipe_on => pipe_on,
			ground_on => ground_on,
			gift_on => gift_on,
            vid_r => vga_r,   -- Mapped directly to physical VGA pins!
			vid_g => vga_g,
			vid_b => vga_b);
